...
## 应用内重跑：4 条残留失败归因与修复（2026-08-02 终版四）

用户在应用内重跑（DeepSeek V4 Flash）：简单 **141/143（98.6%）**、复杂 **27/29（93.1%）**，残留 4 条：

| 用例                      | 现象                                                    | 根因                                                                                                                                                         | 修复                                                                                   |
| ------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------- |
| `util_auto_01`            | `download_internet_file` 真实执行失败（~537ms）         | 依赖 NCBI 外网可达性，与评测目标无关                                                                                                                         | 与 `blast_auto_03/05` 同策略：`assertCallOnly: true`（只评工具选择+URL）               |
| `data_auto_01`            | `export_data({})` 仍 0ms 失败                           | 上一轮修复误用了不存在的 `ExportManager.exportGenBank()`；真实方法是 `exportAsGenBank()` 等                                                                  | `exportData` 切换到真实方法（`exportAsFasta/exportAsGenBank/exportAsGFF/exportAsBED`） |
| `file_auto_complex_02`    | 300s 超时（截图执行 338752ms）                          | `capture_screenshot` 偶发挂起（16:22 重跑 170ms 通过，为环境性卡顿）                                                                                         | 非评分 bug；隔离修复后不再污染后续测试                                                 |
| `protein_auto_complex_01` | 14/15 失败，多了 `capture_screenshot`+`open_image_file` | **跨测试执行数据泄漏**：ChatManager 轮询循环把调用/结果写入共享的 `this.lastExecutionData`，上一测试超时后后台完成的工具调用被写进下一个测试的 executionData | 改为写入请求局部 `executionData`，abort 后后台请求不会污染后续测试                     |
