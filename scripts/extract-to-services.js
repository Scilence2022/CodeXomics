...
const extractedCode = {
  analysis: '', protein: '', file: '', context: '', ui: ''
};
...
for (const [serviceName, methods] of Object.entries(moves)) {
  for (const method of methods) {
    ...
    if (extracted) {
      ...
      const stub = "  " + isAsync + method + "(" + params + ") {\n" +
        "    if (!this.services || !this.services." + serviceName + ") {\n" +
        "      console.error('[ChatManager] " + serviceName + " service not initialized');\n" +
        "      return;\n" +
        "    }\n" +
        "    return " + awaitPrefix + "this.services." + serviceName + "." + method + "(" + delegationArgs + ");\n" +
        "  }";
