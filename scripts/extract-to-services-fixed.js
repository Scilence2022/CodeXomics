...
const moves = {
  ...
  context: [
    'generateSingleToolResponse', 'shouldAllowToolExecution', 'formatToolResult',
    'getCompleteToolContext', 'checkTaskCompletion', 'getBaseSystemMessage',
    'getOptimizedSystemMessage', 'shouldTerminateAfterToolExecution',
    'getAllToolsDetailed', 'addToolResultMessage'
  ],
  ui: [
    'addAlphaFoldSidebarStyles', 'addPDBSidebarStyles', 
    'showChatHistoryModal', 'setupChatDragging', 'showMetabolicPathway'
  ],
...
const extractedCode = {
  analysis: '', protein: '', file: '', context: '', ui: '', intent: ''
};
...
foundMethods.sort(function(a, b) { return b.startIndex - a.startIndex; });
...
for (const m of foundMethods) {
  ...
  const argsList = params.split(',').map(function(p) { return p.split('=')[0].trim(); }).join(', ');
  ...
  const stub = "  " + isAsync + m.name + "(" + params + ") {\n" +
    "    if (!this.services || !this.services." + m.serviceName + ") {\n" +
    "      console.error('[ChatManager] " + m.serviceName + " service not initialized');\n" +
    "      return;\n" +
    "    }\n" +
    "    return " + awaitPrefix + "this.services." + m.serviceName + "." + m.name + "(" + delegationArgs + ");\n" +
    "  }";
...
if (foundMethods.length > 0) {
  const appendToService = function(serviceFile, code) {
    if (!code) return;
    const fp = path.resolve('/Users/song/Github-Repos/GenomeAIStudio_1/src/renderer/modules/chat/services/' + serviceFile);
  ...
  if (extractedCode.intent) {
    const fp = path.resolve('/Users/song/Github-Repos/GenomeAIStudio_1/src/renderer/modules/chat/services/IntentParserService.js');
    if (fs.existsSync(fp)) {
  ...
  if (extractedCode.context) {
    const fp = path.resolve('/Users/song/Github-Repos/GenomeAIStudio_1/src/renderer/modules/chat/services/LLMContextService.js');
    if (fs.existsSync(fp)) {
