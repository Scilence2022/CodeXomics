...
class STRINGNetworkDemo {
    ...
    async fetchRealTimeData(config, logger, validationRules) {
        ...
        try {
            ...
            const avgConfidence = edgeCount > 0 
                ? (data.edges.reduce((sum, e) => sum + (e.confidence || 0), 0) / edgeCount)
                : 0;
    ...
    validateResult(result, rules) {
        ...
        if (rules.minAvgConfidence && edgeCount > 0) {
            ...
            if (avgConfidence < rules.minAvgConfidence) {
                validation.warnings.push(`Low avg confidence: expected >=${rules.minAvgConfidence}, got ${avgConfidence.toFixed(3)}`);
            }
        }
