...
class CodonUsageVerification {
  ...
  verifyChatManagerImplementation() {
    ...
    if (fs.existsSync(chatManagerPath)) {
      ...
      if (content.includes('async codonUsageAnalysis(params)')) {
        ...
        // Check for enhanced features
        if (content.includes('synonymousCodons') && content.includes('rscu') && content.includes('codonPreferences')) {
      ...
      if (content.includes('async genomeCodonUsageAnalysis(params)')) {
        ...
        if (content.includes('gcByPosition') && content.includes('totalGenes') && content.includes('genomeRSCU')) {
      ...
      if (content.includes("case 'codon_usage_analysis':") && content.includes("case 'genome_codon_usage_analysis':")) {
      ...
      if (content.includes('formatToolResultForDisplay') || content.includes('genome_codon_usage_analysis')) {
  ...
  verifyFunctionCallsOrganizer() {
    ...
    if (fs.existsSync(organizerPath)) {
      ...
      if (content.includes("'codon_usage_analysis'") && content.includes("'genome_codon_usage_analysis'")) {
        this.pass('Tool registration', 'Both tools registered in FunctionCallsOrganizer');

        // Verify they're in dataManipulation category
        const dataManipulationMatch = content.match(
          /dataManipulation:\s*{[^}]*functions:\s*\[[^\]]*'codon_usage_analysis'[^\]]*'genome_codon_usage_analysis'[^\]]*\]/s
        );
  ...
  verifyBuiltInToolsIntegration() {
    ...
    if (fs.existsSync(builtinPath)) {
      ...
      if (hasCodonMapping && hasGenomeMapping) {
        ...
        if (
          content.includes("method: 'codonUsageAnalysis'") &&
