...
class SystemIntegration {
  ...
  async generateNonDynamicSystemPrompt(context = {}) {
    try {
      ...
      console.log(
        `🎯 [System Integration] Non-dynamic prompt generated with ${allToolNames.length} tools across ${Object.keys(toolsByCategory).length} categories`
      );
    ...
    try {
      ...
      for (const relevantTool of builtInRelevance) {
        if (relevantTool.confidence >= 0.7) {
          // Include high-confidence built-in tools
          if (relevantTool.is_external) {
            ...
            if (builtInToolInfo) {
              ...
              console.log(
                '🎯 [System Integration] Added built-in tool:',
                relevantTool.name,
                'confidence:',
                relevantTool.confidence
              );
            }
          }
        }
      ...
      if (
        ...
        window.genomeBrowser.chatManager &&
        window.genomeBrowser.chatManager.mcpServerManager
      ) {
        try {
  ...
  async searchTools(keywords, limit = 10) {
    ...
    const matchingTools = allTools.filter(tool => {
      ...
      return searchTerms.some(
        term => toolKeywords.includes(term) || toolDescription.includes(term) || toolName.includes(term)
      );
    });
