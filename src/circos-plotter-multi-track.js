...
class MultiTrackGeneManager {
  ...
  renderGeneTypeTrack(g, genes, geneType, trackConfig, baseRadius, theme) {
    ...
    let renderedGenes = 0;
    genes.forEach(gene => {
      const chr = this.circosPlotter.data.chromosomes.find(
      ...
      g.append('path')
        ...
        .on('mouseover', event => {
          ...
          this.circosPlotter.showTooltip(
            event,
            `
  ...
  renderCDSDensityTracks(g, cdsGenes, baseRadius, theme) {
    ...
    for (let trackIndex = 0; trackIndex < this.cdsDensityTracks; trackIndex++) {
      ...
      const trackRadius =
        baseRadius +
        this.geneTracks.protein_coding.track * (this.circosPlotter.geneHeight + this.trackSpacing) +
