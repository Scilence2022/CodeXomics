...
class Command {
  ...
  async executeWithRetries(context, params, executionOptions) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        const result = await this.executeWithTimeout(context, params, { ...executionOptions, attempt });
  ...
  getStats() {
    return {
      ...this.stats,
      successRate: this.stats.executionCount > 0 ? (this.stats.successCount / this.stats.executionCount) * 100 : 0,
      errorRate: this.stats.executionCount > 0 ? (this.stats.errorCount / this.stats.executionCount) * 100 : 0,
    };
  }
  ...
  toJSON() {
    return {
      ...
      hooks: Object.fromEntries(
        Object.entries(this.hooks).map(([type, hooks]) => [type, hooks.map(h => h.name || 'anonymous')])
      ),
    };
  }
  ...
  debug() {
    return {
      ...
      metadata: this.metadata,
      stats: this.getStats(),
      hooks: Object.fromEntries(Object.entries(this.hooks).map(([type, hooks]) => [type, hooks.length])),
  ...
  getStats() {
    return {
      ...
      categories: Object.fromEntries(
        Array.from(this.categories.entries()).map(([cat, commands]) => [cat, commands.size])
      ),
    };
  }
}
