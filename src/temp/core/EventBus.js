...
class EventBus {
  ...
  wrapListener(listener, options) {
    const wrappedListener = eventData => {
      try {
        ...
        // Call listener
        if (options.async) {
          Promise.resolve(listener(eventData)).catch(error => this.handleListenerError(error, eventData));
        } else {
          listener(eventData);
        }
  ...
  getMetrics() {
    return {
      ...this.metrics,
      totalListeners:
        Array.from(this.events.values()).reduce((total, set) => total + set.size, 0) + this.wildcardListeners.size,
