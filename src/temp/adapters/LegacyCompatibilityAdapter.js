...
class LegacyCompatibilityAdapter {
  ...
  createCompatibilityWrapper(methodName, modernImplementation) {
    return async (...args) => {
      ...
      try {
        ...
        console.error(
          `🚨 [LegacyCompatibilityAdapter] Error in ${methodName} (${useModern ? 'modern' : 'legacy'}):`,
          error
        );
  ...
  shouldUseMoern(moduleOrMethod) {
    ...
    const moduleName = moduleOrMethod.includes('.') ? moduleOrMethod.split('.')[0] : moduleOrMethod;
  ...
  bridgeLegacyEvents() {
    ...
    const legacyEvents = ['sequenceLoaded', 'cursorPositioned', 'sequenceChanged', 'actionQueued', 'actionExecuted'];
