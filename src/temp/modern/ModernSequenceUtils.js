...
class ModernSequenceUtils {
  ...
  registerCommands() {
    ...
    const setCursorCommand = new Command('sequence:setCursor', this.setCursorHandler.bind(this), {
    ...
    const setCursorColorCommand = new Command('sequence:setCursorColor', this.setCursorColorHandler.bind(this), {
    ...
    const handleClickCommand = new Command('sequence:click', this.handleSequenceClickHandler.bind(this), {
    ...
    const renderCommand = new Command('sequence:render', this.renderSequenceHandler.bind(this), {
  ...
  async calculatePositionHandler(context, params) {
    ...
    const position = await this.calculatePositionFromCoordinates(clientX, clientY, viewport, settings, container);
  ...
  handleWindowResize(resizeData) {
    ...
    if (currentCursor.position >= 0) {
      this.taskQueue.add(() => this.recalculateCursorPosition(currentCursor.position), 'high');
    }
  }
  ...
  handleViewportChange(newViewport, oldViewport) {
    ...
    const currentCursor = this.context.getState('sequence:cursor');
    const cursorVisible = currentCursor.position >= newViewport.start && currentCursor.position <= newViewport.end;
