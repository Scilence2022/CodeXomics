...
if (window.benchmarkUI) {
  ...
  if (window.benchmarkUI.manualTestRequiredHandler && window.benchmarkUI.manualTestCompletedHandler) {
    ...
    console.log(
      '⚠️ Manual test handlers may not be set up yet - they will be configured when benchmark interface opens'
    );
  }
  ...
  const testData = {
    ...
    maxScore: 5,
    manualVerification:
      'Please verify: 1) This dialog appears correctly, 2) All elements are visible, 3) Buttons work properly.',
  };
...
if (window.ComprehensiveBenchmarkSuite) {
  try {
    ...
    const fileDialogTests = tests.filter(
      test => test.expectedResult && test.expectedResult.parameters && test.expectedResult.parameters.showFileDialog
    );
