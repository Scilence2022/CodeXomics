...
async function verifyPluginDisabledStateSecurity() {
    ...
    try {
        ...
        if (typeof pluginManager.validatePluginEnabled !== 'function') {
            logTest(
                'validatePluginEnabled method exists',
                false,
                'Method not found in PluginManagerV2'
            );
            return false;
        }
        logTest(
            'validatePluginEnabled method exists',
            true,
            'Security validation method available'
        );
        ...
        if (vizPlugins.length === 0) {
            logTest(
                ...
                false,
                'No visualization plugins found for testing'
            );
            return false;
        }
        ...
        try {
            ...
            logTest(
                'Enabled plugin validation',
                true,
                'Enabled plugin passed validation check'
            );
        } catch (error) {
            logTest(
                'Enabled plugin validation',
                false,
                `Enabled plugin incorrectly blocked: ${error.message}`
            );
        }
        ...
        try {
            ...
            logTest(
                ...
                false,
                'Disabled plugin was allowed to execute (SECURITY FAILURE)'
            );
        } catch (error) {
            const correctError = error.message.includes('disabled') && 
                               error.message.includes('Plugin Management');
            logTest(
                'Disabled plugin validation',
                correctError,
                correctError 
        ...
        try {
            ...
            logTest(
                ...
                correctError,
                correctError
                    ? 'Disabled visualization plugin correctly blocked'
        ...
        if (testResults.failed === 0) {
            ...
            testResults.tests
                .filter(t => !t.passed)
                .forEach(t => console.error(`  - ${t.testName}: ${t.message}`));
        }
