...
class MigrationIntegrationTest {
  ...
  async testCoreInfrastructure() {
    ...
    try {
      ...
      this.addResult('State management', retrievedValue === 42, `Expected 42, got ${retrievedValue}`);

      // Test service registration
      const eventBus = context.getService('eventBus');
      this.addResult('Service registry', eventBus !== null, 'EventBus service available');

      // Test command registry
      const commandRegistry = context.getService('commandRegistry');
      this.addResult('Command registry', commandRegistry !== null, 'Command registry available');
    } catch (error) {
  ...
  async testCompatibilityLayer() {
    ...
    try {
      ...
      this.addResult('Bootstrapper initialization', initResult.success, 'Bootstrapper initialized successfully');

      // Test compatibility adapter creation
      const compatibilityAdapter = initResult.adapter;
      this.addResult('Compatibility adapter creation', compatibilityAdapter !== null, 'Compatibility adapter created');
      ...
      this.addResult('Legacy method wrapping', hasSetCursorPosition, 'Legacy methods wrapped successfully');
  ...
  async testModernModules() {
    ...
    try {
      ...
      this.addResult(
        'ModernActionManager cursor positioning',
      ...
      this.addResult(
        'ModernActionManager paste action',
        pasteResult.success,
      ...
      this.addResult(
        'ModernSequenceUtils cursor setting',
      ...
      this.addResult(
        'ModernSequenceUtils color setting',
        colorResult.success,
  ...
  async testErrorHandling() {
    ...
    try {
      ...
      this.addResult('Invalid parameter handling', !invalidResult.success, 'Invalid cursor position properly rejected');
  ...
  async testCompleteWorkflows() {
    ...
    try {
      ...
      this.addResult('Event propagation', eventReceived, 'Events properly propagated through system');
    } catch (error) {
