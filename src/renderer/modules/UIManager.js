...
class UIManager {
  ...
  initializeSplitter() {
    ...
    document.addEventListener('mousemove', e => {
      ...
      if (
        newGenomeHeight >= minHeight &&
        newGenomeHeight <= maxGenomeHeight &&
        newSequenceHeight >= minHeight &&
        newSequenceHeight <= maxSequenceHeight
      ) {
  ...
  refreshSVGTracks() {
    ...
    console.log(
      '🔄 Refreshing SVG tracks for chromosome:',
      currentChr,
      '| Viewport:',
  ...
  refreshGeneTrackSVG(chromosome, sequence, annotations, operons) {
    ...
    const visibleGenes = annotations.filter(gene => {
      return (
        this.genomeBrowser.shouldShowGeneType(gene.type) && gene.end >= viewport.start && gene.start <= viewport.end
      );
    });
    ...
    this.genomeBrowser.trackRenderer.renderGeneElements(trackContent, visibleGenes, viewport, operons, geneSettings);
  ...
  refreshReadsTrackSVG(chromosome) {
    ...
    const visibleReads = readsData.filter(read => read.end >= viewport.start && read.start <= viewport.end);

    if (visibleReads.length > 0) {
      trackContent.innerHTML = '';
      this.genomeBrowser.trackRenderer.renderReadsTrack(trackContent, visibleReads, viewport);
  ...
  refreshWIGTrackSVG(wigTrackElement, trackName, chromosome) {
    ...
    const visibleData = wigData.filter(point => point.end >= viewport.start && point.start <= viewport.start);
    ...
    if (visibleData.length > 0) {
      ...
      this.genomeBrowser.trackRenderer.createWIGVisualization?.(svg, visibleData, viewport, wigData.color || '#3498db');
  ...
  initializeSidebarSplitter() {
    ...
    const doResize = e => {
      ...
      if (requestedWidth !== newWidth) {
        console.log(
          `🔧 Sidebar resize constrained: ${requestedWidth}px -> ${newWidth}px (max: ${maxWidth}px, screen: ${window.innerWidth}px)`
        );
      }
    ...
    splitter.addEventListener('keydown', e => {
      ...
      if (requestedWidth !== newWidth) {
        console.log(
          `⌨️ Sidebar keyboard resize constrained: ${requestedWidth}px -> ${newWidth}px (max: ${maxWidth}px, screen: ${window.innerWidth}px)`
        );
      }
