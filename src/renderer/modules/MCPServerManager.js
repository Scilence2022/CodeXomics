...
class MCPServerManager {
    ...
    handleClaudeMCPMessage(serverId, event) {
        try {
            ...
            if (data.type === 'tools-list') {
                ...
                console.log(`📝 Received prompts-list from Claude MCP server ${serverId}:`, data.prompts?.length || 0, 'prompts');
    ...
    async requestServerPrompts(serverId) {
        ...
        try {
            ...
            if (server.transportType === 'http' || server.transportType === 'streamable-http' || server.transportType === 'https') {
    ...
    async getPrompt(serverId, promptName, args = {}) {
        ...
        if (server.transportType === 'http' || server.transportType === 'streamable-http' || server.transportType === 'https' || server.protocol === 'streamable-http') {
            const response = await fetch(server.url, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
