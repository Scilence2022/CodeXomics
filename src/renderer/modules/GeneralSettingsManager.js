    ...
    const initializeTabsWithRetry = (attempts = 0, maxAttempts = 5) => {
      ...
      if (firstTab && firstContent) {
        ...
        if (attempts < maxAttempts) {
          ...
          console.log(
            '🔍 [GeneralSettings] All tab contents found:',
  ...
  setupEventListeners() {
    ...
    if (languageSelect) {
      languageSelect.addEventListener('change', async e => {
        ...
        this.showNotification(
          `Language changed to ${e.target.options[e.target.selectedIndex].text}. Some changes may require a restart.`,
          'success'
        );
      });
    }
    ...
    if (maxSequenceLengthSelect) {
      maxSequenceLengthSelect.addEventListener('change', e => {
        this.updateSetting('maxSequenceLength', e.target.value === 'unlimited' ? -1 : parseInt(e.target.value));
      });
    }
    ...
    const featureCheckboxes = [
      'enableGCContent',
      'enableProteinTranslation',
      'enableOperonPrediction',
