...
class MultiFileManager {
  ...
  getSummary() {
    ...
    const totalSize = Array.from(this.fileMetadata.values()).reduce((sum, meta) => sum + (meta.size || 0), 0);

    const variantCount = Array.from(this.vcfFiles.values()).reduce(
      (sum, vcfFile) => sum + (vcfFile.metadata.variantCount || 0),
      0
    );
