...
class BamReader {
    ...
    try {
      ...
      try {
        ...
        console.log('🔍 Header content:', {
          ...
          firstFewRefs: this.header.references
            ? this.header.references.slice(0, 3).map(r => ({ name: r.name, length: r.length }))
            : [],
      ...
      if (this.references.length > 0) {
        console.log(
          `  📋 Sample references:`,
          this.references.slice(0, 5).map(ref => `${ref.name} (${ref.length}bp)`)
        );
      } else {
        console.warn(`  ⚠️ WARNING: No references found - this may cause issues with reads display`);
      }
      console.log(
        `  📈 Total reads: ${this.totalReads > 0 ? this.totalReads.toLocaleString() : 'Will be counted on-demand'}`
      );
      console.log(`  💾 File size: ${this.getFormattedFileSize()}`);
      console.log(
        `  🔍 Index: ${this.hasIndex ? `${this.indexType.toUpperCase()} (${this.getFormattedIndexSize()})` : 'Not available'}`
      );
      ...
      if (this.references.length === 0) {
        console.warn(
          '⚠️ WARNING: BAM file initialized but has no references. This may cause issues with read queries.'
        );
  ...
  async getRecordsForRange(chromosome, start, end, settings = {}) {
    ...
    try {
      ...
      console.log(
        `🔍 [BamReader] Available references (${this.references.length}):`,
      ...
      if (!referenceExists && this.references.length > 0) {
        ...
        const availableChromosomes = this.references
          .slice(0, 5)
          .map(ref => ref.name)
          .join(', ');
        console.warn(
          `   Available chromosomes (first 5): ${availableChromosomes}${this.references.length > 5 ? '...' : ''}`
        );

        // If chromosome doesn't exist and we're not ignoring chromosome, this might be the issue
        if (!settings.ignoreChromosome) {
          console.error(
            `❌ [BamReader] POTENTIAL ISSUE: Chromosome '${chromosome}' not found and ignoreChromosome is false`
          );
        }
      } else {
      ...
      console.log(
        `🔍 [BamReader] Querying region ${chromosome}:${start.toLocaleString()}-${end.toLocaleString()} ${this.hasIndex ? '(indexed)' : '(sequential scan)'}`
      );
      ...
      if (settings.ignoreChromosome) {
        console.log(
          `🔍 [BamReader] IGNORE CHROMOSOME MODE: Fetching records from all chromosomes in position range ${start}-${end}`
        );
      ...
      if (recordsPerBp > 100) {
        console.warn(
          `⚠️ [BamReader] Very high read density: ${recordsPerBp.toFixed(1)} reads/bp for region ${chromosome}:${start}-${end}`
        );
      ...
      if (allRecords.length > EXTREME_DATASET_THRESHOLD) {
        ...
        for (const record of allRecords) {
          const strataIndex = Math.min(Math.floor((record.start - minPos) / strataSize), NUM_STRATA - 1);
        ...
        console.log(
          `📊 [BamReader] Position coverage: ${(coverageRatio * 100).toFixed(1)}% (${sampledMinPos}-${sampledMaxPos})`
        );
      ...
      if (allRecords.length > 0) {
        ...
        for (let i = 0; i < allRecords.length; i += STATS_CHUNK_SIZE) {
          ...
          if (allRecords.length > 100000 && (i / STATS_CHUNK_SIZE) % 50 === 0) {
            console.log(
              `📊 [BamReader] Processing statistics: ${i + chunk.length}/${allRecords.length} records (${(((i + chunk.length) / allRecords.length) * 100).toFixed(1)}%)`
            );
          }
        }
      ...
      if (allRecords.length > CHUNK_SIZE) {
        console.log(
          `🔍 [BamReader] Large dataset detected (${allRecords.length} records), using memory-efficient chunked processing...`
        );
        ...
        for (let i = 0; i < allRecords.length; i += CHUNK_SIZE) {
          ...
          if ((i / CHUNK_SIZE) % 10 === 0) {
            console.log(
              `🔍 [BamReader] Processed ${chunkEnd}/${allRecords.length} records (${((chunkEnd / allRecords.length) * 100).toFixed(1)}%)`
            );
          }
        ...
        console.log(
          `✅ [BamReader] Chunked processing complete: ${reads.length} reads from ${allRecords.length} records`
        );
      } else {
      ...
      console.log(
        `✅ [BamReader] Retrieved ${reads.length.toLocaleString()} reads for region ${chromosome}:${start.toLocaleString()}-${end.toLocaleString()} in ${queryTime.toFixed(1)}ms`
      );
  ...
  convertRecordsToReads(records, chromosome, settings = {}, offset = 0) {
    ...
    for (let i = 0; i < records.length; i++) {
      try {
        ...
        if (mappingQuality < minMappingQuality) {
          lowQualityCount++;
          if (i < 5) {
            // Log first few low quality reads
            console.log(
              `🔍 [BamReader] Filtering low mapping quality read: ${record.name || record.qname} (MQ=${mappingQuality} < ${minMappingQuality})`
            );
          }
          continue;
        }
    ...
    if (reads.length > 0) {
      console.log(
        `🔍 [BamReader] Sample converted reads (first 3):`,
  ...
  getFileInfo() {
    return {
      ...
      performance: {
        ...
        indexUsageRatio:
          this.performanceStats.queryCount > 0
            ? ((this.performanceStats.queriesWithIndex / this.performanceStats.queryCount) * 100).toFixed(1) + '%'
            : '0%',
      },
    };
  }
