...
class FileManager {
  ...
  parseOperonTSV(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (operons.length <= 5) {
        console.log(
          `✅ Parsed operon: ${operonName} (${genes.length} genes, ${strandStr}, confidence: ${confidenceLevelStr})`
        );
      }
    }
  ...
  normalizeOperonData(operon) {
    ...
    return {
      ...
      genes: Array.isArray(operon.genes)
        ? operon.genes
        : operon.genes
          ? operon.genes.split(',').map(g => g.trim())
          : [],
  ...
  async loadFile(filePath, options = {}) {
    ...
    try {
      ...
      if (
        this.genomeBrowser.geneNotesManager &&
        typeof this.genomeBrowser.geneNotesManager.reloadForFile === 'function'
      ) {
        await this.genomeBrowser.geneNotesManager.reloadForFile();
        console.log('📝 Gene notes reloaded for new file');
      }
      if (
        this.genomeBrowser.geneAttachmentsManager &&
        typeof this.genomeBrowser.geneAttachmentsManager.reloadForFile === 'function'
      ) {
...
File size: ${this.currentFile?.info ? (this.currentFile.info.size / (1024 * 1024)).toFixed(1) + ' MB' : 'Unknown'}`;
  ...
  async parseGenBank() {
          ...
          if (currentQualifierKey === 'translation') {
            ...
            if (currentValue.length < 50000) {
              ...
              if (!currentValue.includes('[TRUNCATED]')) {
                currentFeature.qualifiers[currentQualifierKey][lastIndex] =
                  currentValue + ' [TRUNCATED - Content too large]';
              }
            }
          }
        } else {
    ...
    console.log(
      `Final results: ${Object.keys(sequences).length} sequences, ${Object.keys(annotations).length} annotation sets`
    );
  ...
  async parseVCF() {
    try {
      ...
      this.genomeBrowser.updateStatus(
        `✅ VCF file loaded: ${result.metadata.name} (${result.metadata.variantCount} variants)`
      );

      // Update all tabs with new VCF data
      if (this.genomeBrowser.tabManager) {
        this.genomeBrowser.tabManager.onAdditionalFileLoaded(
          'vcf',
          this.genomeBrowser.currentVariants,
...
Original error: ${error.message}`;
  ...
  async parseGFF(options = {}) {
    ...
    if (userChoice === 'merge') {
      ...
      this.genomeBrowser.updateStatus(
        `Loaded GFF file with ${featureCount} features for ${Object.keys(newAnnotations).length} sequence(s). Created new track: ${trackName}`
      );
    }
  ...
  async parseBED(options = {}) {
    ...
    if (userChoice === 'merge') {
      ...
      this.mergeAnnotations(newAnnotations);
      this.genomeBrowser.updateStatus(
        `Loaded BED file with ${featureCount} features for ${Object.keys(newAnnotations).length} chromosome(s). Merged with existing annotations.`
      );
    } else {
      // Create new annotation track
      ...
      this.genomeBrowser.updateStatus(
        `Loaded BED file with ${featureCount} features for ${Object.keys(newAnnotations).length} chromosome(s). Created new track: ${trackName}`
      );
    }
  ...
  mergeAnnotations(newAnnotations) {
    ...
    Object.keys(newAnnotations).forEach(chromosome => {
      ...
      console.log(
        `Merged ${newCount - existingCount} new annotations for chromosome ${chromosome} (total: ${newCount})`
      );
    });
  ...
  async parseWIG() {
    ...
    Object.entries(wigTracks).forEach(([trackName, track]) => {
      ...
      console.log(
        `WIG Track "${trackName}": ${totalDataPoints} data points across ${Object.keys(track.data).length} chromosomes`
      );
    });
  ...
  async parseBigWig() {
    ...
    try {
      ...
      this.genomeBrowser.updateStatus(
        `Added ${newTracksCount} BigWig track(s). Total: ${totalTracksAfterMerge} tracks`
      );
  ...
  createMultipleWIGLoadSummary(results, successCount, errorCount) {
    const totalTracks = results.filter(r => r.status === 'success').reduce((total, r) => total + (r.tracks || 0), 0);
