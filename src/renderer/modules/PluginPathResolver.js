...
class PluginPathResolver {
  ...
  async initialize() {
    ...
    try {
      ...
      this._isElectron = typeof window !== 'undefined' && typeof window.electronAPI !== 'undefined';
   ...
   */
  getAllPluginPaths() {
    return [this.getBuiltinPluginsPath(), this.getUserPluginsPath()];
  }
  ...
  getManifestPath(pluginId, source = 'user') {
    const basePath = source === 'builtin' ? this.getBuiltinPluginsPath() : this.getUserPluginsPath();
