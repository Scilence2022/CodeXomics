...
class VariantAnalyzer {
  ...
  async getVariantLocation(variant) {
    try {
      ...
      if (this.genomeBrowser.currentAnnotations && this.genomeBrowser.currentAnnotations[chromosome]) {
        ...
        const overlappingFeatures = annotations.filter(feature => position >= feature.start && position <= feature.end);
    ...
    try {
      ...
      if (this.genomeBrowser.currentAnnotations && this.genomeBrowser.currentAnnotations[chromosome]) {
        ...
        const overlappingFeatures = annotations.filter(feature => {
          return (
            (varStart >= feature.start && varStart <= feature.end) ||
  ...
  async analyzeGeneImpact(variant, gene) {
    try {
      ...
      // Get gene sequence
      const geneSequence = await this.genomeBrowser.getSequenceForRegion(chromosome, geneStart, geneEnd);
      ...
      if (gene.type === 'CDS') {
        return await this.analyzeCDSImpact(workingSequence, mutatedSequence, workingRelativePos, variant, gene);
      } else {
        ...
        if (codonPosition < originalProtein.length && codonPosition < mutatedProtein.length) {
          ...
          if (originalAA !== mutatedAA) {
            if (mutatedAA === '*') {
              ...
              details.push(
                `🔄 MISSENSE: ${originalAA} → ${mutatedAA} (${this.getAminoAcidName(originalAA)} → ${this.getAminoAcidName(mutatedAA)})`
              );
            }
          } else {
            severity = 'Low';
