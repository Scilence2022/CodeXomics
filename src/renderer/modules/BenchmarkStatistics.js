...
class BenchmarkStatistics {
  ...
  calculatePerformanceStatistics(testResults) {
    ...
    const responseTimes = testResults.map(t => t.metrics?.responseTime).filter(rt => rt !== undefined);
    const tokenCounts = testResults.map(t => t.metrics?.tokenCount).filter(tc => tc !== undefined);
  ...
  getTValue(confidence, degreesOfFreedom) {
    ...
    const tTable = {
      0.95: { 1: 12.706, 2: 4.303, 5: 2.571, 10: 2.228, 20: 2.086, 30: 2.042, 60: 2.0, 120: 1.98, Infinity: 1.96 },
      0.99: { 1: 63.657, 2: 9.925, 5: 4.032, 10: 3.169, 20: 2.845, 30: 2.75, 60: 2.66, 120: 2.617, Infinity: 2.576 },
    };
  ...
  extractErrorPattern(error) {
    ...
    const patterns = [/(\w+) not found/i, /Invalid (\w+)/i, /Cannot (\w+)/i, /Failed to (\w+)/i, /(\w+) timeout/i];
  ...
  getMostCommon(frequency) {
    ...
    return Object.entries(frequency).reduce((max, [key, value]) => (value > max.value ? { key, value } : max), {
      key: null,
      value: 0,
    });
  }
  ...
  calculateAverageComplexity(testResults) {
    const complexities = testResults.map(t => t.metrics?.instructionComplexity).filter(c => c !== undefined);
