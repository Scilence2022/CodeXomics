...
class ShortTermMemory {
     ...
     */
    async initialize() {
        // No specific initialization needed for short-term memory
    ...
    getToolUsagePattern() {
        ...
        this.toolCallHistory.forEach(call => {
            if (!patterns.has(call.toolName)) {
                patterns.set(call.toolName, {
                    count: 0,
                    avgExecutionTime: 0,
                    successRate: 0,
                    lastUsed: null,
            ...
            const pattern = patterns.get(call.toolName);
            pattern.count++;
            pattern.totalExecutionTime += call.executionTime;
            ...
            pattern.lastUsed = call.timestamp;
        });
    ...
    getSessionStats() {
        ...
        const successfulCalls = this.toolCallHistory.filter(call => call.success).length;
        ...
        return {
            ...
            averageExecutionTime: totalCalls > 0 ? 
                this.toolCallHistory.reduce((sum, call) => sum + call.executionTime, 0) / totalCalls : 0
        };
    }
    ...
    async search(functionName, parameters) {
        ...
        for (const call of this.toolCallHistory) {
            if (call.functionName === functionName && this.matchesQuery(call, parameters)) {
                results.push({
                    ...call,
                    score: this.calculateRelevanceScore(call, functionName, parameters)
                });
            }
        }
    ...
    matchesQuery(call, query) {
        if (typeof query === 'string') {
            return call.functionName === query || 
                   call.parameters?.includes?.(query) ||
                   call.context?.includes?.(query);
        }
        return true;
    }
     ...
     */
    calculateRelevanceScore(call, functionName, parameters) {
        let score = 0;
        ...
        // Parameter similarity
        const paramSimilarity = this.calculateParameterSimilarity(call.parameters, parameters);
        score += paramSimilarity * 5;
        
        // Recency bonus
        const age = Date.now() - call.timestamp;
        const recencyBonus = Math.max(0, 1 - (age / (60 * 60 * 1000))); // Decay over 1 hour
        score += recencyBonus * 2;
    ...
    getStats() {
        return {
            ...
            patterns: this.getToolUsagePattern().size
        };
    }
}
