...
class ActivationEventsService {
  ...
  async triggerEvent(event) {
    ...
    const requests = extensions.map(extId => new ActivationRequest(extId, ActivationTrigger.Event, event));
  ...
  async _performStartupActivation() {
    ...
    for (const extensionId of immediateExtensions) {
      const request = new ActivationRequest(extensionId, ActivationTrigger.Startup, '*');
      this._queue.enqueue(request);
    }
    ...
    if (deferredExtensions.size > 0) {
      ...
      for (const extensionId of deferredExtensions) {
        const request = new ActivationRequest(extensionId, ActivationTrigger.StartupFinished, 'onStartupFinished');
        this._queue.enqueue(request);
      }
  ...
  async activateDependency(extensionId, dependencyId) {
    ...
    const request = new ActivationRequest(dependencyId, ActivationTrigger.Dependency, `dependency:${extensionId}`);
  ...
  getStats() {
    return {
      ...
      averageActivationTime:
        this._stats.totalActivations > 0 ? this._stats.totalActivationTime / this._stats.successfulActivations : 0,
