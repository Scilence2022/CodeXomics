  ...
  getMetrics() {
    return {
      ...
      averageOperationTime:
        this._metrics.operationCount > 0 ? this._metrics.totalCpuTime / this._metrics.operationCount : 0,
    };
  }
...
class ExtensionHost {
    ...
    try {
      switch (message.type) {
        ...
        case MessageType.Ping:
          ...
          this._protocol.handleResponse(
            new ExtensionHostMessage(MessageType.Pong, message.id, { timestamp: Date.now() })
          );
          break;
   ...
   */
  getStats() {
    const activatedCount = Array.from(this._extensions.values()).filter(e => e.isActivated).length;
