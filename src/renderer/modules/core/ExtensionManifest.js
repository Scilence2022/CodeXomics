...
const ManifestSchema = {
    ...
    optional: [
        'main', 'browser', 'activationEvents', 'contributes', 'dependencies',
        'devDependencies', 'extensionDependencies', 'extensionPack',
        'extensionKind', 'capabilities', 'keywords', 'categories',
        'icon', 'license', 'repository', 'bugs', 'homepage', 'qna',
        'sponsor', 'markdown', 'pricing', 'badges', 'galleryBanner',
        'preview', 'enabledApiProposals', 'scripts', 'engines'
    ],
    types: {
        name: 'string',
        version: 'string',
...
class ManifestValidator {
    ...
    validate(manifest) {
        ...
        for (const [field, expectedType] of Object.entries(ManifestSchema.types)) {
            if (raw[field] !== undefined && !this._validateType(raw[field], expectedType)) {
                errors.push(new ManifestValidationError(
                    `Invalid type for ${field}: expected ${expectedType}`,
                    field,
                    'INVALID_TYPE'
                ));
            }
    ...
    _validateActivationEvents(events) {
        ...
        for (const event of events) {
            ...
            const isValid = validPrefixes.some(vp => 
                vp === prefix || event.startsWith(vp)
            );
...
class ManifestBuilder {
     ...
     * Set basic info
     */
    name(name) { this._manifest.name = name; return this; }
    version(version) { this._manifest.version = version; return this; }
    displayName(name) { this._manifest.displayName = name; return this; }
    description(desc) { this._manifest.description = desc; return this; }
    publisher(pub) { this._manifest.publisher = pub; return this; }
    author(author) { this._manifest.author = author; return this; }
