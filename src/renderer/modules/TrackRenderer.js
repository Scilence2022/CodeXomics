...
class TrackRenderer {
  constructor(genomeBrowser) {
    ...
    // Canvas renderers backing the Feature Glyph Legend previews (destroyed
    // when the legend closes or is rebuilt)
    this.glyphLegendRenderers = [];
  ...
  isCircularModeEnabled(settings = null) {
    ...
    return Boolean(
      settings?.circularMode || genesSettings?.circularMode || this.genomeBrowser?.navigationManager?.circularMode
    );
  }
  ...
  createAnnotationTrack(chromosome, annotationTrack) {
    ...
    const trackContent = this.createTrackContent(
      settings.height || this.trackConfig.genes?.defaultHeight || 120,
      chromosome
    );
    track.appendChild(trackContent);
  ...
  renderGeneElementsSVG(trackContent, geneRows, viewport, operons, layout, settings) {
    ...
    const defs = document.createElementNS('http://www.w3.org/2000/svg', 'defs');
    GeneShapeCreators.createSpecializedGradients(defs); // Add specialized gradients for different gene types
    svg.appendChild(defs);
  ...
  createSVGGeneElement(gene, viewport, operons, rowIndex, layout, settings, defs, containerWidth) {
    ...
    const isSpecialized = GeneShapeCreators.shouldUseSpecializedShape(geneType);
    ...
    if (elementWidth > 30) {
      const geneText = GeneShapeCreators.createSVGGeneText(this, gene, elementWidth, elementHeight, settings);
      if (geneText) {
    ...
    GeneShapeCreators.addSVGGeneInteraction(this, geneGroup, gene, operons, operonInfo, rowIndex);
...
This action cannot be undone.`;
    ...
    try {
      ...
      if (isAnnotationTrack) {
        ...
        this.genomeBrowser.annotationTracks = this.genomeBrowser.annotationTracks.filter(t => t.id !== fileId);
  ...
  /**
   * Open the Feature Glyph Legend modal.
   * The legend itself lives in tracks/FeatureGlyphLegend.js so it can draw its
   * previews with the same renderers and settings this track uses.
   */
  openFeatureGlyphLegend() {
    FeatureGlyphLegend.open(this);
  }
  ...
  collectSettingsFromModal(trackType, modal) {
    ...
    switch (trackType) {
      ...
      case 'reads': {
        ...
        settings.samplingThreshold = Math.min(
          100000,
          Math.max(1000, parseInt(modal.querySelector('#readsSamplingThreshold').value) || 10000)
        );
        settings.samplingMode = modal.querySelector('#readsSamplingMode').value || 'percentage';
        settings.samplingPercentage = Math.min(
          100,
          Math.max(1, parseInt(modal.querySelector('#readsSamplingPercentage').value) || 20)
        );
        settings.samplingCount = Math.min(
          50000,
          Math.max(1000, parseInt(modal.querySelector('#readsSamplingCount').value) || 5000)
        );
  ...
  updateSamplingPercentage(percentage, fileId = null) {
    ...
    console.log(
      `🎲 [TrackRenderer] Updating sampling percentage to ${percentage}%${fileId ? ` for file ${fileId}` : ''}`
    );
  ...
  updateGeneTrackSVG(chromosome, sequence, annotations, operons) {
    ...
    if (svgContainer) {
      ...
      GeneShapeCreators.updateSVGTextForResize(svgContainer, containerWidth);
