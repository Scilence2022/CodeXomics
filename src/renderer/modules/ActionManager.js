...
class ActionManager {
    ...
    adjustPendingActionPositionsEnhanced(executedAction, startIndex, executionActionsCopy) {
        ...
        if (!chromosomeLength) {
            console.warn(`⚠️ [ActionManager] No chromosome length available for ${chromosome}, skipping position adjustments`);
            return;
        }
        ...
        for (let i = startIndex; i < executionActionsCopy.length; i++) {
            ...
            if (adjustmentResult.action === 'skip') {
                ...
                this.applyActionPositionAdjustment(pendingAction, adjustmentResult.newStart, adjustmentResult.newEnd, positionShift);
                adjustedCount++;
                console.log(`🔧 [ActionManager] Adjusted action ${pendingAction.id} position: ${pendingStart}-${pendingEnd} → ${adjustmentResult.newStart}-${adjustmentResult.newEnd}`);
            }
        }
        
        console.log(`🔧 [ActionManager] Enhanced position adjustment complete: ${adjustedCount} adjusted, ${failedCount} failed`);
    }
