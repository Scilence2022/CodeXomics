...
class TabManager {
  ...
  createNewTab(title = null, specificPosition = null) {
    ...
    if (specificPosition) {
      this.updateTabPositionVisualization(
        tabId,
        specificPosition.chromosome,
        specificPosition.start,
        specificPosition.end
      );
  ...
  createTabState(tabId, title, specificPosition = null) {
    ...
    return {
      ...
      trackSettings: this.genomeBrowser.trackRenderer
        ? this.deepCopyTrackSettings(this.genomeBrowser.trackRenderer.trackSettings)
        : {},
      headerStates: this.genomeBrowser.trackRenderer
  ...
  saveCurrentTabState() {
    ...
    try {
      ...
      console.log(
        `Saved state for tab: ${this.activeTabId} at position ${tabState.currentChromosome}:${tabState.currentPosition.start}-${tabState.currentPosition.end}`
      );
  ...
  restoreTabState(tabId) {
    ...
    try {
      ...
      console.log(
        `Restored state for tab: ${tabId} at position ${tabState.currentChromosome}:${tabState.currentPosition.start}-${tabState.currentPosition.end}`
      );
    } catch (error) {
  ...
  onGenomeLoaded(genomeData, filename) {
    ...
    if (this.genomeBrowser.windowId && typeof ipcRenderer !== 'undefined') {
      ...
      console.log(
        `📋 [TabManager] Reported genome name to main process: ${genomeName} (window: ${this.genomeBrowser.windowId})`
      );
    }

    // Also notify standalone MCP Bridge if available
    if (window.mcpBridge) {
      window.mcpBridge.reportGenomeLoaded(genomeName);
    }
  ...
  restoreFromCache(tabId) {
    ...
    if (
      cached.position.start !== tabState.currentPosition?.start ||
  ...
  setupTrackDragAndDrop() {
    ...
    trackItems.forEach((item, index) => {
      ...
      item.addEventListener('drop', e => {
        ...
        if (draggedElement && draggedElement !== item) {
          ...
          console.log(
            `🔧 [TabManager] Drop operation: ${dropPosition}, draggedIndex: ${draggedIndex}, targetIndex: ${targetIndex}`
          );
    ...
    console.log(
      '✅ [TabManager] Enhanced track drag-and-drop functionality initialized for',
      trackItems.length,
      'items'
    );
  }
  ...
  updateTrackVisibilityControls() {
    try {
      ...
      const trackTypes = ['genes', 'gc', 'variants', 'reads', 'proteins', 'primers', 'sequence', 'actions', 'blast'];
      ...
      trackTypes.forEach(trackType => {
        ...
        const toggleBtn = document.getElementById(
          `toggle${trackType.charAt(0).toUpperCase() + trackType.slice(1)}Track`
        );
      ...
      const sidebarTrackTypes = [
        ...
        { type: 'primers', id: 'sidebarTrackPrimers' },
  ...
  enforeCacheLimit() {
    ...
    const sortedEntries = Array.from(this.tabCache.entries()).sort((a, b) => a[1].timestamp - b[1].timestamp);
  ...
  updateTabPositionVisualization(tabId, chromosome, start, end) {
    ...
    try {
      ...
      console.log(
        `Updated position visualization for tab ${tabId}: ${chromosome} ${startPercent.toFixed(1)}%-${endPercent.toFixed(1)}% (width: ${displayWidth.toFixed(1)}%)`
      );
    } catch (error) {
  ...
  restoreSidebarPanelStates(tabState) {
    ...
    if (variantPanel && tabState.sidebarPanels.variantDetailsSection) {
      if (
        tabState.sidebarPanels.variantDetailsSection.visible &&
        tabState.sidebarPanels.variantDetailsSection.content
      ) {
