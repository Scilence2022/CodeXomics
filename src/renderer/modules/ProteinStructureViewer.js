...
class ProteinStructureViewer {
  ...
  showUnifiedProteinSearchDialog() {
    ...
    const existingDialogs = document.querySelectorAll(
      '.unified-protein-search-dialog, .protein-search-dialog, .alphafold-search-dialog'
    );
  ...
  initializeNGLViewer(viewerWindow, pdbData, proteinName, pdbId) {
    try {
      ...
      if (!viewerWindow.NGL) {
        ...
        this.handleNGLError(
          viewerWindow,
          'NGL library failed to load. This may be due to network issues or browser security restrictions.'
        );
        return;
      }
      ...
      try {
        ...
        this.handleNGLError(
          viewerWindow,
          `Failed to create 3D viewer: ${stageError.message}. This may be due to WebGL limitations on your system.`
        );
        return;
      }
      ...
      const loadingElement = viewerWindow.document.getElementById('loading');
      if (loadingElement) {
        loadingElement.textContent = 'Error initializing 3D viewer: ' + error.message + ' (Check console for details)';
      }
    }
  }
  ...
  async requestMCPTool(toolName, parameters) {
    ...
    throw new Error(
      'MCP Server not connected or not available. Please ensure the MCP server is running and connected.'
    );
  }
        ...
        if (connectedCount === 0) {
          ...
          try {
            ...
            resultsDiv.innerHTML =
              '<div class="error">MCP server not connected. Please connect to MCP server first.</div>';
            return;
          }
        }
      }
