...
class CanvasSequenceRenderer {
  ...
  getCDSRegions() {
    ...
    return annotations.filter(
      annotation =>
        annotation.type === 'CDS' && annotation.start < this.viewport.end && annotation.end > this.viewport.start
    );
  }
  ...
  calculateMetrics() {
    ...
    this.charHeight =
      Math.abs(sampleMetrics.actualBoundingBoxAscent) + Math.abs(sampleMetrics.actualBoundingBoxDescent);
  ...
  render() {
    ...
    if (this.sequence.length * this.charWidth < availableWidth) {
      ...
      console.log(
        `🔍 [CanvasSequenceRenderer] Short sequence detected, stretching: ${effectiveCharWidth.toFixed(2)}px per char`
      );
    } else {
