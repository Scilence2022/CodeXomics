...
class PluginManagerV2 {
  ...
  async _initializeNewArchitecture() {
    ...
    try {
      ...
      if (typeof ActivationEventsService !== 'undefined') {
        ...
        this.activationService.setActivationHandler(this._handleExtensionActivation.bind(this));
  ...
  validatePluginEnabled(pluginId, plugin = null) {
    ...
    if (targetPlugin.enabled === false) {
      ...
      throw new Error(`Plugin "${pluginName}" is disabled. Please enable it in Plugin Management before use.`);
    }
  ...
  async executeVisualizationTool(toolName, parameters = {}) {
    ...
    try {
      ...
      if (plugin.executor && typeof plugin.executor === 'function') {
        ...
        throw new Error(
          `Plugin ${pluginId} does not have a valid visualization method (executor/renderNetwork/visualize)`
        );
      }
