...
class ConnectionHealthMonitor extends EventEmitter {
  ...
  recordToolExecution(connectionId, toolName, responseTime, success = true) {
    ...
    const totalTime = Array.from(this.connections.values()).reduce((sum, c) => sum + c.stats.totalResponseTime, 0);
    const totalExecs = Array.from(this.connections.values()).reduce((sum, c) => sum + c.stats.toolExecutions, 0);
  ...
  getConnectionHealth(connectionId) {
    ...
    return {
      ...
      stats: {
        ...connection.stats,
        averageResponseTime:
          connection.stats.toolExecutions > 0
            ? connection.stats.totalResponseTime / connection.stats.toolExecutions
            : 0,
        errorRate: connection.stats.toolExecutions > 0 ? connection.stats.errors / connection.stats.toolExecutions : 0,
      },
    };
  }
  ...
  getSystemHealth() {
    ...
    return {
      ...
      metrics: {
        ...
        errorRate:
          this.metrics.totalToolExecutions > 0
