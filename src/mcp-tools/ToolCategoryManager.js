...
class ToolCategoryManager {
    ...
    canExecuteInContext(toolName, context) {
        ...
        switch (context) {
            case 'remote':
                return mode === this.executionModes.SERVER_ONLY || 
                       mode === this.executionModes.HYBRID;
                
            case 'local':
                return mode === this.executionModes.CLIENT_ONLY || 
                       mode === this.executionModes.HYBRID;
    ...
    getToolMetadata(toolName) {
        ...
        return {
            name: toolName,
            executionMode: mode,
            canExecuteRemotely: mode === this.executionModes.SERVER_ONLY || 
                               mode === this.executionModes.HYBRID,
            requiresClient: mode === this.executionModes.CLIENT_ONLY || 
                           mode === this.executionModes.HYBRID,
